...
from swarmauth.middleware import TokenIssuer, UsageTracker, require_capability, verify_and_check
...
def test_verify_and_check_success():
    ...
    token = issuer.issue(iss="a", sub="tool:b", caps=["tool:b"])
...
def test_verify_and_check_missing_capability():
    kp = KeyPair.generate()
    issuer = TokenIssuer(kp)
    token = issuer.issue(iss="a", sub="tool:b", caps=["tool:read_invoice"])
...
def test_usage_tracker_max_calls():
    kp = KeyPair.generate()
    issuer = TokenIssuer(kp)
    token = issuer.issue(iss="a", sub="tool:b", caps=["tool:b"], constraints=Constraints(max_calls=1))
...
def test_usage_tracker_budget():
    kp = KeyPair.generate()
    issuer = TokenIssuer(kp)
    token = issuer.issue(iss="a", sub="tool:b", caps=["tool:b"], constraints=Constraints(max_amount_usd=100.0))
    tracker = UsageTracker()
...
def test_require_capability_decorator_blocks_without_token():
    kp = KeyPair.generate()

    @require_capability("tool:process_payout", issuer_public_key=kp.public_bytes)
...
def test_require_capability_decorator_allows_with_valid_token():
    kp = KeyPair.generate()
    issuer = TokenIssuer(kp)
    token = issuer.issue(iss="a", sub="tool:process_payout", caps=["tool:process_payout"])

    @require_capability("tool:process_payout", issuer_public_key=kp.public_bytes, audience="tool:process_payout")
    def process_payout(destination_account, amount_usd):
        return "executed"
...
def test_require_capability_decorator_enforces_allowed_params():
    kp = KeyPair.generate()
    issuer = TokenIssuer(kp)
    token = issuer.issue(
        iss="a",
        sub="tool:process_payout",
        caps=["tool:process_payout"],
